...
## 2.3.1-1.20.1 - Forge Backport

### Adicionado

- Backport para Forge 1.20.1 dos recursos lancados na linha 1.21.1 ate `2.3.1`.
- Log de blocos, containers, itens, gamemode, teleportes e troca de dimensao.
- Filtros por jogador/UUID, mundo/dimensao e comandos ignorados.
- Logs em texto ou JSON Lines (`jsonl`) e indice global em `_global`.
- Alertas ativos por categoria, jogador observado ou comando observado.
- Broadcast de alertas para operadores e envio opcional via Discord webhook.
- Estatisticas de sessao e comandos `/adminlogger reload`, `/adminlogger status` e `/adminlogger stats`.
- Arquitetura refatorada em pacotes por responsabilidade, igual a linha recente do mod.

### Ajustado

- Codigo adaptado de NeoForge para Forge 1.20.1 mantendo Java 17.
- Build atualizado para Forge `1.20.1-47.4.20`.
- Metadados, README e config de exemplo atualizados para a linha Forge.
